...
if [ ${dimension} == "2D" ]; then
	python3 generate2DStencil_cuda.py ${neighbourhood} ${orientation} ${coefficients} ${range} ${xCoarsening} ${yCoarsening} ${alignment} ${CLsize} > kernels.cu
	if [ $? -ne 0 ]; then
		err "generate2DStencil_cuda.py failed, exiting"
	fi
	...
	python3 generate3DStencil_cuda.py ${neighbourhood} ${orientation} ${coefficients} ${range} ${xCoarsening} ${yCoarsening} ${zCoarsening} ${alignment} ${CLsize} > kernels.cu
	if [ $? -ne 0 ]; then
		err "generate3DStencil_cuda.py bench failed, exiting"
	fi
