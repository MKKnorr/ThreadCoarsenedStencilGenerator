...
void stencilBenchmark::initGPU(){
	...
	gpuErrchk(hipSetDevice(0)); //just use first gpu
	hipDeviceProp_t dProps;
	gpuErrchk(hipGetDeviceProperties(&dProps, 0));
...
void stencilBenchmark::initBenchmark(){
	...
	size_t freeGPUMem;
	size_t totalGPUMem;
	hipMemGetInfo(&freeGPUMem, &totalGPUMem);
	...
	gpuErrchk(hipMalloc(&d_array0, sizeof(double)*N));
	gpuErrchk(hipMalloc(&d_array1, sizeof(double)*N));
